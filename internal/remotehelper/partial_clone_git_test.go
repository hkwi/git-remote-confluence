...
func TestGitPartialCloneAndStrictFetch(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/rest/api/content/3/child/page", "/rest/api/content/3/child/attachment":
	...
	if output, err := runGit(tmp, "-c", "confluence.allowPartialClone=false", "clone", remoteURL, filepath.Join(tmp, "strict")); err == nil || !strings.Contains(output, "HTTP 404") {
	...
	metadata := string(readCloneFile(t, partial, "1.yml"))
	if !strings.Contains(metadata, "children:\n  - \"2\"\n") || !strings.Contains(metadata, "skipped_children:\n  - \"3\"\n") {
	...
	if _, err := os.Stat(filepath.Join(partial, "1", "3.md")); !os.IsNotExist(err) {
